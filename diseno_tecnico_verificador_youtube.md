...
- Brief por archivo o extracción con LLM: en v1 el brief se carga en un **formulario manual estructurado**. Subir archivo (`.txt`/`.docx`/PDF) + extracción automática queda para una versión futura.
...
1. **Entrada** — saber qué canales monitorear (archivo parseado en el cliente → grilla editable) y contra qué brief verificar (formulario manual). Todo lo escribe la UI por RLS, sin API backend.
...
Las partes pesadas (transcripción, razonamiento del LLM) se delegan a servicios externos vía API. La persistencia y la integración entre componentes viven en Supabase (Postgres), que actúa como contrato compartido entre el backend y la interfaz. **No se usan APIs de Google ni OAuth.** El **frontend** escribe toda la configuración por RLS y lee los resultados; el **backend** (callback de WebSub + cron tick) produce los resultados — los dos se encuentran solo en Supabase, no se llaman entre sí.

---

## 3. Entrada: canales (archivo + grilla) y brief

Dos entradas distintas, las dos **gestionadas desde el frontend contra Supabase (RLS)**. No se usan Google APIs ni OAuth, ni hace falta una API backend para la entrada: la UI escribe la config directo en la base. El backend solo *reacciona* (resuelve y suscribe) cuando procesa los canales (ver sección 4 y el cron tick de la sección 10).

### 3.1 Canales: subir archivo → grilla editable (todo en el frontend)

El usuario sube un archivo con las URLs de los canales (CSV / `.xlsx`, p. ej. una planilla descargada de Google Sheets). **El frontend lo parsea del lado del cliente** y lo carga en una **grilla editable**, donde puede **agregar, eliminar y corregir filas a mano**. La fuente de verdad es la propia app (la grilla). También se puede pegar/escribir URLs directamente, sin archivo.

Las filas se escriben en la tabla `channels` **por RLS** (la UI, con la *anon key*), en estado `resolution_status='unresolved'`. No pasan por ninguna API backend.

### 3.2 Reconciliación al importar / editar (en el frontend)

Cada vez que se sube un archivo nuevo, **reemplaza el conjunto activo reconciliando** (nunca borra, para no perder historial). El frontend, que conoce la grilla actual y la lista nueva, computa el diff y aplica por RLS:

- Canal que **ya existía** (match por URL) → sigue **activo** (no se re-resuelve).
- Canal **nuevo** → se inserta como `unresolved`.
- Canal que estaba activo y **ya no viene** → `is_active = false`. Si reaparece → se reactiva.

Cada import queda registrado en `import_runs`.

### 3.3 Resolución de canal (URL → channel_id) — la hace el backend

La detección por WebSub necesita el `channel_id` (`UC...`). El **cron tick del backend** (sección 10) busca los canales `unresolved`, los resuelve y los suscribe al WebSub. La resolución es exacta y barata: `channels.list` con `forHandle` cuesta **1 unidad** de quota (vs `search` por nombre suelto, que cuesta 100). Si no se puede resolver, el canal queda `unresolved` y aparece marcado en la grilla para corregir a mano. Es la **única** parte de la entrada que toca el backend (porque necesita la YouTube API key).

### 3.4 Brief: formulario manual estructurado (sin LLM)

El brief se carga en un **formulario estructurado** dentro de la app: nombre del juego, link esperado, código, y **qué requisitos verificar** (R1–R5, con su `spec`). **No hay subida de archivo ni extracción con LLM** — el usuario completa los campos a mano. Como el brief se arma una sola vez por campaña, un formulario es simple, determinístico y sin dependencias.

Esto se guarda en `campaigns` + `requirements` **por RLS** desde la UI (sin backend de por medio). El usuario elige los requisitos que aplican; no hace falta que estén los cinco.
...
- **Resolución por el cron tick:** la resolución de canales `unresolved` + suscripción WebSub la hace el **cron tick** del backend (sección 10), que en cada corrida procesa lo pendiente. No hay *database webhook* (a este volumen, un tick cada pocos minutos es más simple que la infra de un webhook y la latencia es irrelevante).
...
- **RLS:** cada usuario lee/escribe solo lo de sus organizaciones. La UI gestiona **toda la configuración** (canales, campañas, requisitos/brief, asignaciones, reviews) con la *anon key* gobernada por RLS — **sin pasar por ninguna API backend**. El backend escribe los **resultados** (videos, transcripts, verificaciones) con la *service_role key* (bypassea RLS), seteando siempre el `org_id` correcto. Así el backend no necesita una API autenticada.
...
4. **Gestión de campañas:** crear/editar campaña, fijar el plazo (`ends_at` obligatorio), **cargar el brief en un formulario manual** (nombre del juego, link, código), **elegir qué requisitos verificar** y **asignar canales** a la campaña. Todo se guarda por RLS.
5. **Importación de canales (grilla):** subir archivo (CSV/`.xlsx`, parseado en el cliente) o editar a mano la grilla de URLs; ver canales **no resueltos** para corregir; historial de imports. Las filas se escriben por RLS.
...
Regla mental: **el frontend escribe la config y lee los resultados (todo por RLS); el backend produce los resultados y maneja YouTube.** Supabase es el contrato compartido, y **el frontend y el backend no se hablan entre sí** — se encuentran solo en la base.
...
- **Gestiona toda la configuración** (grilla de canales con parseo del archivo y reconciliación del lado del cliente, campañas, brief por formulario manual, asignaciones, reviews) escribiendo con RLS por org. **No llama a ninguna API backend.**
- **Lee y muestra** los resultados de verificación; no calcula ni inventa la lógica de verificación (eso lo produce el backend).

**Backend / motor** (Python; **sin API autenticada**):
- **Callback de WebSub** (lo único que escucha HTTP): suscripción, validación de firma, recepción de avisos.
- **Cron tick** (un solo job periódico) que en cada corrida: resuelve canales `unresolved` (+ suscribe WebSub), renueva leases por expirar, reintenta transcripts vencidos (backoff) y marca incumplimientos por plazo.
- Motor de verificación: chequeos determinísticos + llamada a la LLM con structured output (el prompt de verificación, que se itera y se mide contra el set dorado), **una verificación por campaña matcheada**.
- Harness de evaluación. (Notificaciones → Fase 5.)

> El backend **no** expone endpoints para el frontend: la entrada (canales y brief) la escribe la UI por RLS, y la resolución/suscripción la hace el cron tick. La LLM se usa solo para la **verificación** (R3/R4), no para el brief.
...
**Opción B — servicio Python dedicado (recomendada).** Un solo servicio Python corre todo el motor, en **dos piezas**: el **callback de WebSub** (lo único que escucha HTTP) y un **cron tick** que hace todo el mantenimiento (resolución de canales, backoff de transcript, renovación de leases, revisor de plazos) y la verificación. Supabase queda como base de datos + auth; el frontend React le pega directo para leer y escribir config por RLS.

**Recomendación: opción B.** Como en v1 el transcript se obtiene con la librería de Python, ya necesitás sí o sí un servicio Python. Tener *todo* el motor ahí es más simple de razonar, testear y desplegar. Y como **el backend no expone API** (la config la escribe la UI por RLS), su superficie HTTP se reduce a un solo endpoint público (el callback de WebSub).

**Despliegue:** corre en un **VPS de Hostinger** con **Docker** (`docker compose`). **Caddy** sirve el frontend estático (mismo dominio → sin CORS) y hace de reverse proxy con HTTPS automático del **callback de WebSub** (`/websub/callback`). El cron tick corre como tarea programada del VPS. No hay API autenticada ni JWT: el frontend solo habla con Supabase. El detalle está en `docs/requisitos-despliegue.md`.
...
|---|---|---|
| Interfaz web | React + Vite + `@supabase/supabase-js`, construida con Claude Code | Gestiona toda la config por RLS y lee resultados; no llama a ninguna API backend |
| Backend / motor | Python (callback WebSub + cron tick) vía Claude Code | Procesos stateful, transcript en Python; sin API autenticada |
| Entrada | Canales: archivo CSV/`.xlsx` parseado en el cliente → RLS. Brief: formulario manual → RLS | Sin Google APIs ni OAuth; sin backend de por medio |
| Detección | YouTube WebSub + Data API v3 | Push en tiempo real |
| Transcript | `youtube-transcript-api` (MIT), detrás de interfaz abstracta | v1 sin proxies; cambiar de motor sin reescribir |
| LLM | API OpenAI con structured output | Solo para la **verificación** (R3/R4); veredicto JSON auditable |
| Persistencia | Supabase (Postgres + Auth + RLS) | Contrato compartido; el único punto donde se cruzan frontend y backend |
...
**Fase 2 — Resolución de canales (backend).** Normalización de referencias (URL/handle → ref) y resolución a `channel_id` con la YouTube Data API. Es la única pieza de la entrada que necesita el backend (la YouTube key); la usa el cron tick. El parseo del archivo, la reconciliación y el brief viven en el frontend (Fase 4).

**Fase 3 — Detección automática (WebSub) + cron tick.** Suscripción por canal, validación de `hub.challenge` y firma, dedup por `videoId`, asociación video→campaña(s), verificación multi-campaña, y el **cron tick** que orquesta resolución + backoff de transcript + renovación de leases + revisor de plazos. Sin database webhook ni API autenticada.

**Fase 4 — Interfaz web + multi-tenancy (Claude Code).** Las vistas en React (sección 8) sobre el esquema de Supabase, con **auth + organización personal** y **RLS por org**. Incluye el parseo del archivo de canales y la reconciliación **del lado del cliente**, y el **brief por formulario manual**. Toda la escritura es por RLS (sin API backend). Supabase Auth + skill `frontend-design`.
...
| Canal no resuelto | Estado `unresolved` (lo intenta el cron tick) + corrección manual en la grilla |
...
| Brief cargado con error | Es un formulario manual; el usuario edita y corrige los campos antes de guardar |
...
1. El usuario se registra (se crea su organización personal), importa sus canales a la grilla (archivo parseado en el cliente o a mano) y crea una campaña: completa el brief en un **formulario manual** (juego, link, código), elige los requisitos, asigna canales y fija el plazo. Todo se escribe por RLS. El **cron tick** del backend resuelve los canales `unresolved` y los suscribe al WebSub.
